...
function add(name, ratings, lockstatus, comments) {                               //START OF ADD FUNCTION
 ...
 menname.style.display = "none";
  ...
  function expandcollapse(){
      ...
      if(div.style.gridTemplateRows == "1fr 0.5fr 0.5fr 1fr 1fr 1fr 1fr 1fr")    //12-delete 13-edit
      {
        div.style.gridTemplateRows = "1fr 0.5fr 0.5fr 1fr";
        div.children[15].style.display = "none";
          div.children[16].style.display = "none";
      ...
      else if(div.style.gridTemplateRows = "1fr 0.5fr 0.5fr 1fr")
      {
        console.log(div.style.gridTemplateRows);
...
function save() {
  ...
  mentees[i].remove();
    for(i=0;i<localStorage.getItem("length");i++){
  reconstruct(objarray[i].name, objarray[i].rat, true, objarray[i].comments);
